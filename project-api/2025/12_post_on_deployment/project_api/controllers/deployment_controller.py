...
def app_create_deployment_id(user, project_name, body: dict):
    logger.debug(f"> app_create_deployment_id: user={user}, project={project_name}")
    if connexion.request.is_json:
        body = connexion.request.get_json()
        try:
            logger.debug(f"- app_create_deployment_id: Incoming raw body:\n{json.dumps(body, indent=2)}")
        except Exception:
            logger.debug("- app_create_deployment_id: Incoming body could not be pretty-printed (non-serializable)")
        logger.debug(f"- app_create_deployment_id: Incoming body keys: {list(body.keys())}")
        applications = body.get("applications")
        deployment = body.get("deployment")
        
        if not deployment:
            logger.warning("- app_create_deployment_id: Missing 'deployment' in request body")
            return response_error(msg="Missing deployment in the request body", status_code=400)
        if not isinstance(deployment, dict):
            logger.error(f"- app_create_deployment_id: Invalid 'deployment' type: {type(deployment)}")
            raise TypeError("deployment must be an object")
        if 'gateway' not in deployment:
            logger.error("- app_create_deployment_id: Missing 'deployment.gateway' array")
            raise KeyError("deployment.gateway missing")
        if 'leaf' not in deployment:
            logger.error("- app_create_deployment_id: Missing 'deployment.leaf' array")
            raise KeyError("deployment.leaf missing")
        logger.debug(f"- app_create_deployment_id: Deployment.gateway count: {len(deployment.get('gateway', []))}")
        logger.debug(f"- app_create_deployment_id: Deployment.leaf count: {len(deployment.get('leaf', []))}")
        ...
        if applications:
            logger.debug(f"- app_create_deployment_id: Applications received: {len(applications)}")
            for idx, app in enumerate(applications):
                logger.debug(f"- app_create_deployment_id: applications[{idx}] => {app}")
                if 'uuid' not in app:
                    logger.error(f"- app_create_deployment_id: applications[{idx}] missing required 'uuid' field: {app}")
                ...
                if existing_app:
                    ...
                    new_apps.append(app)
        logger.info(f"- app_create_deployment_id: UUID mapping created for {len(uuid_mapping)} apps; new apps to create: {len(new_apps)}")
        logger.debug(f"- app_create_deployment_id: UUID mapping detail: {uuid_mapping}")
        ...
        for gateway in deployment['gateway']:
            if gateway['application'] in uuid_mapping:
                ...
                logger.warning(f"- app_create_deployment_id: Non-existing gw application UUID referenced: {gateway['application']}")
        ...
        for leaf in deployment['leaf']:
            if 'datalogging' in leaf:
                if leaf['datalogging']['application'] in uuid_mapping:
                    ...
                    logger.warning(f"- app_create_deployment_id: Non-existing datalogging application UUID referenced: {leaf['datalogging']['application']}")
                ...
                if set(leaf['datalogging']['supported_applications']).issubset(set(curr_app_uuids)):
                    logger.debug(f'- app_create_deployment_id: - leaf-datalogging-supported_applications: {leaf['datalogging']['supported_applications']}')
                else:
                    logger.warning(f"- app_create_deployment_id: Non-existing supported_applications application UUID referenced: {leaf['datalogging']['supported_applications']}")
            ...
            if 'inference' in leaf:
                if leaf['inference']['application'] in uuid_mapping:
                    ...
                    logger.warning(f"- app_create_deployment_id: Non-existing inference application UUID referenced: {leaf['inference']['application']}")
                ...
                if set(leaf['inference']['supported_applications']).issubset(set(curr_app_uuids)):
                    logger.debug(f'- app_create_deployment_id: - leaf-inference-supported_applications: {leaf['inference']['supported_applications']}')
                else:
                    logger.warning(f"- app_create_deployment_id: Non-existing inference supported_applications UUID referenced: {leaf['inference']['supported_applications']}")
        ...
        logger.debug(f"- app_create_deployment_id: - Deployment object to deserialize: {json.dumps(deployment, indent=4)}")
        new_deployment = NewDeployment.from_dict(deployment)  # noqa: E501
        logger.debug(f'- app_create_deployment_id: - new_deployment: {new_deployment}')
        apps = []
        if new_apps:
            for app in new_apps:
        ...
        try:
            logger.debug(
                f"- app_create_deployment_id: Calling project_repo.create_deployment with: name={new_deployment.display_name}, "
            ...
            logger.warning(f"- app_create_deployment_id: Deployment already exists: {new_deployment.display_name}")
            return response_error(msg="Deployment resource already existing", status_code=400)
        logger.info(f"- app_create_deployment_id: Deployment created: name={new_deployment.display_name}, apps_created={len(apps)}")
