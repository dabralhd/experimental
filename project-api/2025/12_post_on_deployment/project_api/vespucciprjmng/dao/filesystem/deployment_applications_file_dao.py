...
from project_api.vespucciprjmng.domain.deployment import Bluest_V3_Payload, Bluest_V3_PayloadDecodingSchemaInstance
...
class DeploymentApplicationFileDAO(DeploymentDAO):
    ...
    def __deserialize_application(self, json_application_obj) -> Application:
        ...
        if "bluest_v3_payload" in json_application_obj:
            payload = json_application_obj.get("bluest_v3_payload", {})
            logger.debug(f'collecting bluest_v3_payload \nbluest_v3_payload: {json.dumps(payload)}')
            if payload != {}:            
                p = Bluest_V3_Payload()
                ...
                p.decoding_schema = [ Bluest_V3_PayloadDecodingSchemaInstance(
                        component=ds.get("component", ""), telemetry=ds.get("telemetry", ""), type_=ds.get("type", "")
                    )
                    for ds in schema_list if isinstance(ds, dict)
                ] 
            application_domain_obj.bluest_v3_payload = p
        else:
            application_domain_obj.bluest_v3_payload             = None
        logger.debug(f'< DeploymentApplicationFileDAO.De__deserialize_application')
    ...
    def __serialize_application(self, application_obj: Application) -> Any:
        ...
        payload = application_obj.bluest_v3_payload if hasattr(application_obj, "bluest_v3_payload") else None
        logger.debug(f'DeploymentApplicationFileDAO.__serialize_application: Serialize bluest_v3_payload fields')
        if payload is not None:
            try:
                logger.debug(f'serializing bluest_v3_payload')
                json_application_domain_obj["bluest_v3_payload"] = payload.to_dict()
            except AttributeError:
                logger.warning('bluest_v3_payload object missing to_dict; skipping serialization')
