...
def convert_application(application=VespucciDeploymentApplication):
    deployment_application=APIDeploymentApplication(
                            ...
                            bluest_v3_payload = application.bluest_v3_payload
...
def convert_deployment (deploy : VespucciDeployment):
    ...
    logger.debug(f'> vespucci_to_controller_model_converters.convert_deployment')
    ...
    logger.info(f'- parsing deploy.leaf_devices')
    for lf in deploy.leaf_devices:
        logger.debug(f'- lf.inference_app: {lf.inference_app}')
        logger.debug(f'- lf.datalogging_app: {lf.datalogging_app}')
        
        inference_supported_applications = [y for y in lf.inference_app.get('supported_applications')]
        dtlg_supported_applications = [y for y in lf.datalogging_app.get('supported_applications')]
        # logger.debug(f'- lf.inference_app: {lf.inference_app}')
        # logger.debug(f'- lf.inference_app: {lf.datalogging_app}')
        
        # lf_inf_app = lf.inference_app
        # if lf_inf_app and lf_inf_app.get('supported_applications'):
        #     inference_supported_applications = [y for y in lf_inf_app.get('supported_applications')]
        # else:
        #     inference_supported_applications = []            

        # lf_dtlg_app = lf.datalogging_app        
        # if lf_dtlg_app and lf_dtlg_app.get('supported_applications'):    
        #     dtlg_supported_applications = [y for y in lf_dtlg_app.get('supported_applications')]
        # else:
        #     dtlg_supported_applications = []
        
        #logger.debug(f'- inference_supported_applications: {inference_supported_applications}\ndtlg_supported_applications: {dtlg_supported_applications}')
        ...
        lfNew = APIDevice(device_id=lf.device_id,
            description=lf.description,
            display_name=lf.display_name,
            gateway_id=lf.gateway_id,
            inference=DeviceApplication(application=lf.inference_app["application"], firmware_update=lf.inference_app["firmware_update"], models=infModels, supported_applications=inference_supported_applications),
            datalogging=DeviceApplication(application=lf.datalogging_app["application"], firmware_update=lf.datalogging_app["firmware_update"], models=None, supported_applications=dtlg_supported_applications)            
        )
        leaves.append(lfNew)
    ...
    logging.debug(f'< vespucci_to_controller_model_converters.convert_deployment')
