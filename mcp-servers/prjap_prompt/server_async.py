...
# from fastmcp.prompts.message import Message, PromptResult, UserMessage, SystemMessage
from mcp.types import Message, PromptResult, UserMessage, SystemMessage
import asyncio

mcp_prjapi = FastMCP('prjapi-mcp-server')
...
logger = setup_logger(name='logger_mcp_prjapi', level=logging.DEBUG)
...
@mcp_prjapi.prompt()
def project_discovery(title: str, description: str, priority: str = "Medium") -> PromptResult:
    """
    Generates a structured request to the AI to create a new GitHub issue 
    using the 'github_api' tool.

    This prompt guides the model's behavior for a multi-step task.
    
    Args:
        title: The short, descriptive title for the new issue.
        description: The detailed body content for the issue.
        priority: The priority level (Low, Medium, High). Defaults to Medium.
    """
    # 2. Define the core instructions for the LLM as a System Message.
    system_instruction = (
        "You are an Issue Creation Expert. You have access to the 'github_api' tool "
        "which has a function 'create_github_issue(title, body, labels)'. "
        "Your task is to call this tool with the provided user inputs."
    )
    
    # 3. Define the actual content/user-facing message. This is what includes the template variables.
    # We use the Message helper class for structuring conversation turns.
    # The final message guides the LLM to call the appropriate tool.
    user_request_message = (
        f"The user wants to create a new issue with the following details:\n\n"
        f"Title: {title}\n"
        f"Description: {description}\n"
        f"Priority: {priority}\n\n"
        f"Please generate the function call to the 'github_api' tool to fulfill this request."
    )
    
    # 4. Return the list of messages that constitute the prompt to the LLM.
    # This structure is often called a 'multi-message' prompt.
    return [
        Message(content=system_instruction, role="system"),
        Message(content=user_request_message, role="user"),
    ]


if __name__ == "__main__":
    # Initialize and run the server
    mcp_prjapi.run(transport='stdio')    
